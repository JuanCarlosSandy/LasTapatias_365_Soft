                ...
                <Toast :breakpoints="{'920px': {width: '100%', right: '0', left: '0'}}" style="padding-top: 40px;"></Toast>
                    ...
                    <template #header>
                        <div v-if="tipoMenu === 2" class="header-container">
                            ...
                            <p v-if="cantidadSeleccionada" class="cantidad-texto">
                                Cantidad seleccionada: <strong>{{ cantidadSeleccionada.label }}</strong>
                            </p>
                        </div>
                    </template>
                            ...
                            <template #grid="slotProps">
                                ...
                                <template #footer>
                                    ...
                                    <Button 
                                        v-else 
                                        icon="pi pi-plus" 
                                        class="p-button-sm p-button-success rounded-bottom-right" 
                                        :disabled="categoria_general === 'comidas' && !cantidadSeleccionada" 
                    ...
                    <!----><div v-show="mostrarDelivery" class="p-grid p-fluid">
                        ...
                        <div class="p-col-12 p-md-4">
                            <div class="p-inputgroup">
                                ...
                                <InputNumber  class="p-inputnumber-sm" placeholder="Tarifa Delivery" v-model="tarifa_delivery" ref="direccionDelivery" />
                                <span class="p-inputgroup-addon">
                                    <Checkbox v-model="tarifaDeliveryIncluida" :binary="true" />
                                </span>
                            </div>
                        </div>
...
export default {
    ...
    methods: {
        ...
        /*agregarDelivery(idRol) {
            if (this.justifyOptions.length <= 2) {
                if (idRol === 1) {
                    let  delivery = {icon: 'pi pi-car', label: 'Delivery', value: 'Entregas'}
                    this.justifyOptions.push(delivery);
                }
            }
        },*/
    ...
    editarMenu(event) {
        ...
        agregarDetalleModal(data) {
            ...
            if (productoEnCarrito) {
                productoEnCarrito.cantidad += 1; // Aumentar la cantidad en uno
                this.$toast.add({
                    severity: 'info',
                ...
                this.$toast.add({
                    ...
                    detail: `${data.nombre.toUpperCase()} ha sido agregado con 1 unidad.`,
                    life: 1000
                });
            }
            
            // Actualizar la factura si el producto ya existe
            if (productoEnFactura) {
                productoEnFactura.cantidad += 1; // Incrementar de 1 en 1
                productoEnFactura.subTotal = productoEnFactura.precioUnitario * productoEnFactura.cantidad;
            } else {
                // Si el producto no está en la factura, agregarlo con 1 unidad
                me.arrayFactura.push({
                    actividadEconomica: 749000,
                    codigoProductoSin: data.codigoProductoSin,
                    codigoProducto: data.codigo,
                    descripcion: data.nombre,
                    cantidad: 1, // Usamos la cantidad inicial de 1
                    unidadMedida: 57,
                    precioUnitario: data.precio_venta,
                    montoDescuento: 0,
                    subTotal: data.precio_venta * 1, // Subtotal inicial con 1 unidad
                    numeroSerie: null,
                    numeroImei: null
                });
            }
        ...
        async obtenerDatosUsuario() { 
            try {
                ...
                // Asignar tipoMenu desde la respuesta del backend
        ...
        async registrarVenta(idtipo_pago) {
            ...
            try {
                ...
                const ventaResponse = await axios.post('/venta/registrar', {
                    ...
                    'tarifaDelivery': this.tarifa_delivery,
                    'tarifaDeliveryIncluida': this.tarifaDeliveryIncluida,
                    'primer_precio_cuota': this.primer_precio_cuota,
                    'cliente': this.cliente,
                    'documento': this.documento,
                    'tipoEntrega': tipoEntregaValor,
                    'idMesa': idMesa, 
                    'observacion': this.cantidadSeleccionada,
                ...
                if (ventaResponse.data.id > 0) {
                    ...
                    if(tipoEntregaValor === 'D'){
                        ...
                        try {
                            ...
                            //console.log(numDelivery);
                        } catch (error) {
                            console.error('Error al recuperar el teléfono:', error);
                        }
...
/* Estilo adicional para los botones del SelectButton */
